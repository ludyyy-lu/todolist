...
func GetTags(c *gin.Context) {
	...
	if err := config.DB.Where("user_id = ?", userID).Find(&tags).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "查询失败"}) // 返回错误信息
		return
	}
	c.JSON(http.StatusOK, tags) // 返回标签列表
}
...
func DeleteTag(c *gin.Context) {
	...
	if tag.UserID != userID { // 判断标签是否属于当前用户
		c.JSON(http.StatusForbidden, gin.H{ // 返回错误信息
			"error": "权限不足", // 返回错误信息
		})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{ // 返回成功信息
		"message": "标签已删除", // 返回成功信息
	})
}
...
func GetTodoTags(c *gin.Context) {
	...
	if err := config.DB.Preload("Tags").Where("id = ? AND user_id = ?",todoID, userID).First(&todo).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{ // 返回错误信息
			"error": "任务不存在", // 返回错误信息
		})
		return
	}
...
func RemoveTodoTag(c *gin.Context) {
	...
	if err := config.DB.Model(&todo).Association("Tags").Delete(&tag); err!= nil { // 删除关联关系
		c.JSON(http.StatusInternalServerError, gin.H{ // 返回错误信息
			"error": "删除失败", // 返回错误信息
		})
		return
	}
